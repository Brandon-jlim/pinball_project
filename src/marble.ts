...
export class Marble {
  ...
  constructor(physics: IPhysics, order: number, max: number, name?: string, weight: number = 1, controlId?: string) {
    ...
    const physicsBias = Math.max(0, Math.min(1, (this.weight - 0.1) / 1.0));
    const density = 0.8 + physicsBias * 2.7;
    const restitution = 0.3 - physicsBias * 0.18;
    const friction = 0.15 + physicsBias * 0.2;
    const linearDamping = 0.18 - physicsBias * 0.1;
    const angularDamping = 0.2 - physicsBias * 0.12;
