...
import type { IPhysics, MarbleBodyOptions } from './IPhysics';
...
export class Box2dPhysics implements IPhysics {
  ...
  createMarble(id: number, x: number, y: number, options: MarbleBodyOptions = {}): void {
    ...
    fixtureDef.set_density(options.density ?? 1);
    fixtureDef.set_restitution(options.restitution ?? 0.25);
    fixtureDef.set_friction(options.friction ?? 0.2);

    body.CreateFixture(fixtureDef);
    body.SetLinearDamping(options.linearDamping ?? 0.05);
    body.SetAngularDamping(options.angularDamping ?? 0.1);
    body.SetAwake(false);
  ...
  setMarbleVelocity(id: number, x: number, y: number): void {
    ...
    marble.SetLinearVelocity(new this.Box2D.b2Vec2(x, y));
  }
  ...
  setMarbleAngularVelocity(id: number, value: number): void {
    ...
    marble.SetAngularVelocity(value);
  }
